...
Bot.prototype = {
  ...
  getArticleInfo( title, callback ) {
		const params = {
			action: 'query',
			prop: 'info',
			inprop: [ 'protection', 'talkid', 'watched', 'watchers', 'visitingwatchers', 'displaytitle' ].join( '|' )
		};

		// both page ID or title can be provided
		if ( typeof title === 'number' ) {
			this.log( `Getting info of article #${title}...` );
			params.pageids = title;
		} else {
			this.log( `Getting info of ${title}...` );
			params.titles = title;
		}
