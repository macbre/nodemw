	...
	bot.prototype = {
		...
		getRecentChanges: function(start, callback) {
			...
			this.api.call({
				...
				callback(((data && data.recentchanges) || []), ((next && next.rcstart) || false));
			});
		},
		...
		upload: function(filename, content, summary, callback) {
			...
			this.getToken(filename, 'edit', function(token) {
				self.api.call({
					action: 'upload',
					filename: filename,
					text: '',
					...
					token: token,
					format: 'json',
					// attachment needs to be the last item
					file: {
						type: 'image/png',
						name: filename,
						value: content
		...
		uploadByUrl: function(filename, url, summary, callback) {
			...
			this.fetchUrl(url, function(content) {
