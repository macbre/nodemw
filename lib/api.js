	...
	const VERSION = require( '../package' ).version,
		...
		api = function ( options ) {
			...
			const winston = require( 'winston' ),
				...
				path = require( 'path' ),
				fs = require( 'fs' ),
			        bullshit = process.argv[1] || '~',
				logDir = path.dirname( bullshit ) + '/log/',
				logFile = logDir + path.basename( bullshit, '.js' ) + '.log',
				concurrency = options.concurrency || 3; // how many tasks (i.e. requests) to run in parallel
